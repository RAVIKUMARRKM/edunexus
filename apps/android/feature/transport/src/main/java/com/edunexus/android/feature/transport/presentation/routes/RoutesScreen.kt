...
import com.edunexus.android.feature.transport.presentation.components.RouteCard

/**
 * Routes List Screen
 * Displays all transport routes with search, filter operations
 */
...
fun RoutesScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.effect.collect { effect ->
            when (effect) {
                is RoutesEffect.NavigateToDetail -> onNavigateToDetail(effect.routeId)
                is RoutesEffect.ShowToast -> {
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    // Refresh button
                    IconButton(onClick = { viewModel.handleIntent(RoutesIntent.RefreshRoutes) }) {
                        Icon(
        ...
        Column(
            ...
            SearchBar(
                ...
                onValueChange = {
                    searchQuery = it
                    viewModel.handleIntent(RoutesIntent.SearchRoutes(it))
                },
            ...
            SingleSelectFilterChips(
                ...
                onChipSelected = { chip ->
                    ...
                    viewModel.handleIntent(RoutesIntent.FilterByStatus(newFilter))
                },
                ...
                when (val state = uiState) {
                                ...
                                RouteCard(
                                    ...
                                    onClick = {
                                        viewModel.handleIntent(
                                            RoutesIntent.NavigateToDetail(route.id)
                                        )
                                    }
                                )
                            }
                    ...
                    is RoutesUiState.Error -> {
                        EmptyState(
                            ...
                            actionLabel = "Retry",
                            onActionClick = {
                                viewModel.handleIntent(RoutesIntent.RefreshRoutes)
                            }
                        )
                    }
                }
